...
var cssCore = function(testCss) {
...
}(document.createElement("ComicView").style), translate = function(core) {
  if(core !== "") {
...
}(cssCore), animationFrame = function() {
  var lastTime = 0, i = 0, vendors = ["webkit", "moz", "ms"], len = vendors.length;
  ...
  if(!window.requestAnimationFrame || !cancelAnimationFrame) {
    window.requestAnimationFrame = function(callback, element) {
      var currTime = +new Date, timeToCall = Math.max(0, 16.7 - currTime + lastTime), id = window.setTimeout(function() {
 ...
 get:function(settings) {
  if(settings.url.indexOf("User") !== -1 && !cookie("skey")) {
    settings.callback('{"status":-99,"state":-99}');
    return
  }
  var xhr = this._createXMLHttpRequest(), data = settings.data || "", async = settings.async || true;
...
post:function(settings) {
  var xhr = this._createXMLHttpRequest(), data = settings.data || "";
  if(data) {
...
}, docEle = document.documentElement, on = function() {
  ...
  return function(o, from, to, time) {
    ...
    var os = o.style, timeStamp = +new Date, end = timeStamp + time, opacityDiff = to - from, _fade = function() {
      var diff = new Date - timeStamp, percent = ~~(diff / time * 100) / 100, opacityNow = from + opacityDiff * percent;
      if(opacityNow > 1) {
        opacityNow = 1
      }else {
        if(opacityNow < 0) {
...
}(document.createElement("ComicView").style), osWidth = docEle.clientWidth || window.innerWidth, osHeight = docEle.clientHeight || window.innerHeight, isIE = function(ua) {
  return ua.indexOf("MSIE") !== -1
}(navigator.userAgent), isMac = function(ua) {
  return ua.indexOf("Mac OS") !== -1
}(navigator.userAgent), isLocalStorageNameSupported = function() {
  try {
...
}(navigator.userAgent), mouse = {down:mobileDevice ? "touchstart" : "mousedown", move:mobileDevice ? "touchmove" : "mousemove", up:mobileDevice ? "touchend" : "mouseup", click:mobileDevice ? "touchend" : "click"}, makeArray = function(obj) {
  return Array.prototype.slice.call(obj, 0)
}, mainView = $$("mainView"), body = document.body || document.getElementsByTagName("body")[0], checkScrollChange = function() {
...
if(cssCore !== "") {
  transToX = function(o, x, t, fn, n) {
    var s = o.style, c = "translate(" + x + "px, 0) translateZ(0)";
  ...
  transToY = function(o, y, t) {
    var s = o.style, c = "translate(0," + y + "px) translateZ(0)";
    s[cssCore + "TransitionDuration"] = t + "ms";
    s[cssCore + "Transform"] = c
  }
}else {
...
!function() {
  var viewTheme = cookie("theme"), themeControl = $$("themeControl"), themeChild = themeControl.children, uav = $$("userAvatar"), assignSysAvatar = function(theme) {
    if(theme === "dark") {
      if(USER === undefined) {
...
var crossPage = +cookie("crossPage"), cp = $$("crossPage"), roastBarShrink = $$("roastBarShrink"), roastBarWrap = $$("roastBarWrap");
if(crossPage) {
...
!function() {
  ...
  function c(c, d) {
    ...
    var g = d.debug, h = d.openTag, i = d.closeTag, j = d.parser, k = d.compress, l = d.escape, m = 1, p = {$data:1, $filename:1, $utils:1, $helpers:1, $out:1, $line:1}, q = "".trim, s = q ? ["$out='';", "$out+=", ";", "$out"] : ["$out=[];", "$out.push(", ");", "$out.join('')"], t = q ? "$out+=text;return $out;" : "$out.push(text);", u = "function(){var text=''.concat.apply('',arguments);" + t + "}", v = "function(filename,data){data=data||$data;var text=$utils.$include(filename,data,$filename);" + 
    t + "}", w = "'use strict';var $utils=this,$helpers=$utils.$helpers," + (g ? "$line=0," : ""), x = s[0], y = "return new String(" + s[3] + ");";
...
function ScrollBar(options) {
  ...
  sk.onmousedown = function(e) {
    ...
    removeClass(c, "moved");
    e = e || window.event;
    start = {X:e.clientX, Y:e.clientY, time:+new Date};
    delta = {}
  };
  ...
  return{init:function(width, height) {
    H = c.offsetHeight || H;
    h = fx ? fx : height - fd;
    h = H - 1 < h ? H : h;
    S = h - fh;
...
var scaleRate = 1, _isbottom = false, goScroll = function(e) {
  ...
  var isFromScroll = false, direct, nowScale, thisScroll, o;
  e = e || window.event;
  ...
  if(isFromScroll) {
    ...
    if(thisScroll === "roast-bar") {
      ...
      (new Function("return " + thisScroll))().wheelMove(direct * (isMac ? 0.3 : 1))
    }
  }else {
    if(crossPage) {
      if(!e.ctrlKey) {
...
}, scrollCatalogue = ScrollBar({contain:catalogueList, wrap:catalogueListWrap, scrollBg:catalogueScrollBg, scrollBlock:catalogueScrollBlock, factHeightDiff:190, scrollBarHeightDiff:0}), scrollBookshelf, scrollToolPage, _t, _thisStep;
...
if(!crossPage) {
  var endDiff = 0, scrollInterval = null, smoothScroll = function(from, to, t) {
    endDiff = endDiff + to;
    var timeStamp = +new Date, nowDiff = endDiff, _smoothScroll = function() {
      var now = +new Date;
  ...
  document.onkeydown = function(e) {
    e = e || window.event;
    var code = e.keyCode, moveLongRange = function(dir) {
      var scrollTop = getScrollTop();
      if(e.preventDefault) {
        e.preventDefault()
      }else {
...
!function() {
  ...
  var _cssCore = cssCore.toLowerCase(), checkScreen = function() {
    mainView.style.height = osHeight - 120 + "px"
  };
  on(fullscreenChange, mouse.click, function() {
    var requestFullScreen = _cssCore === "ms" ? "msRequestFullscreen" : _cssCore + "RequestFullScreen", cancelFullScreen = _cssCore === "ms" ? "msExitFullscreen" : _cssCore + "CancelFullScreen";
...
!function() {
  var comicContain = $$("comicContain"), comicContainCross = $$("comicContainCross"), roastBarShrink = $$("roastBarShrink"), roastStopElem = null, roastStopItem = null, plusOneWrap = $$("plusOneWrap"), roastStopState = body.className.indexOf("roast-dm") === -1 ? 1 : 2, roastPlusOne = function(e0, clientX, clientY) {
    var i = document.createElement("i"), doc = docEle;
    ...
    anchor = {x:e0.pageX - 9, y:e0.pageY - 14};
    ...
    var parent = e.parentNode, gParent = parent.parentNode, roastSeqString = "", roastDataItem = null, roastStopState = body.className.indexOf("roast-dm") === -1 ? 1 : 2, promptTop = $$("promptTop"), roastGoodPrompt = function() {
      ...
      sendPgv("NEW.AC_VIEW.NEWDM.DIANZAN_TUCAO");
      if(!USER) {
        return toLogin()
      }
      if(type === 1) {
      ...
      if(type === 1 && roastDataItem.hasGood || type === 2 && tar.getAttribute("data-good")) {
        ...
        var roastId = tar.getAttribute("data-roast-id"), goodObj = tar.getElementsByTagName("i")[0], clientX = e0.pageX ? null : e0.clientX, clientY = e0.pageY ? null : e0.clientY;
        if(type === 1) {
        ...
        ajax.get({url:"http://ac.qq.com/ComicView/updateRoastGoodorBad/?comic_id=" + ID + "&roast_id=" + roastId + "&type=1", callback:function(data) {
          ...
          if(+data.ret === 2) {
            ...
            if(+data.status === -99) {
              toLogin();
              sendPgv("NEW.AC_VIEW.NEWDM.DENGLU_DIANZAN")
            }
          }
        }})
      }
    }, dmGood = function(tar, e0, type) {
      sendPgv("NEW.AC_VIEW.NEWDM.DIANZAN_DANMU");
      if(!USER) {
        return toLogin()
      }
      if(type === 1) {
      ...
      if(type === 1 && roastDataItem.hasGood || (type === 2 || type === 3) && tar.getAttribute("data-good")) {
        ...
        var roastId = tar.getAttribute("data-roast-id"), goodObj = tar.getElementsByTagName("span")[0], clientX = e0.pageX ? null : e0.clientX, clientY = e0.pageY ? null : e0.clientY;
        if(type === 1) {
          roastDataItem.hasGood = 1
        }else {
          if(type === 2) {
      ...
      if(!parent.getAttribute("data-user-hide")) {
        ...
        sendPgv("NEW.AC_VIEW.NEWDM.CHADIAO")
      }
    };
    if(parent.className === "for-roast") {
      ...
      if(gParent.className === "for-roast") {
        if(roastStopState === 1) {
        ...
        if(parent.className === "own-roast") {
          ...
          if(gParent.className === "own-roast") {
            if(roastStopState === 2) {
              dmGood(parent, e0, 2)
            }else {
              if(roastStopState === 1) {
    ...
    e = e || window.event;
    e = e.target || e.srcElement;
    var parent = e.parentNode, grandParent = parent.parentNode, roastItem = null;
    if(roastStopState === 2) {
      if(parent.className === "for-roast" || parent.className === "own-roast" || parent.className === "adv-roast") {
        ...
        var disAll = e.offsetWidth + parent.offsetWidth + (parent.className === "adv-roast" ? 300 : 100), lastPass = e.getAttribute("data-timepass") ? ~~e.getAttribute("data-timepass") : 0, timePass = +new Date - e.getAttribute("data-timestart") + lastPass, disPass = disAll * timePass / DANMU_TIME * -1;
        ...
        if(grandParent.className === "for-roast" || grandParent.className === "own-roast") {
          if(roastStopElem) {
            roastRestart()
          }
          ...
          var disAll = parent.offsetWidth + grandParent.offsetWidth + 100, lastPass = parent.getAttribute("data-timepass") ? ~~parent.getAttribute("data-timepass") : 0, timePass = +new Date - parent.getAttribute("data-timestart") + lastPass, disPass = disAll * timePass / DANMU_TIME * -1;
      ...
      if(roastStopState === 1) {
        if(parent.className === "for-roast") {
          ...
          if(grandParent.className === "for-roast") {
            if(roastStopElem) {
              return
            }
            roastStopElem = parent;
  ...
  if(comicContain) {
    ...
    comicContain.onmousedown = function(e) {
      ...
      var scrollTop = getScrollTop(), now = +new Date, from = {X:e.clientX, Y:e.clientY};
      ...
      comicContain.onmouseup = function(e) {
        e = e || window.event;
        var toX = from.X - e.clientX, toY = from.Y - e.clientY, diff = +new Date - now, v = toY / diff;
        if(Math.abs(toX) > 350) {
          if(toX < 0) {
            if(NEXT_CHAPTER) {
              window.location.href = makeUrl(ID, NEXT_CHAPTER, 1)
  ...
  if(comicContainCross) {
    ...
    on(comicContainCross, mouse.down, function(e) {
      ...
      if(!mobileDevice && e.button === 2 && tar.tagName.toLowerCase() === "img" && !roastState) {
        var s = tar.style, p = tar.parentNode.parentNode.style;
      ...
      var now = +new Date, from = {X:e.clientX, Y:e.clientY}, move = function(e) {
        e = e || window.event;
        if(e.preventDefault) {
          e.preventDefault()
        }
        ...
        var toX = from.X - e.clientX, diff = +new Date - now, v = toX / diff;
...
window.noPrev = function() {
  ...
  var dialogHtml = "", data = {title:"\u63d0\u793a", emotion:"^_^", text:"\u4e3b\u4eba\uff0c\u8fd9\u662f\u672c\u4f5c\u54c1\u7684\u7b2c\u4e00\u7ae0\uff0c\u6ca1\u6709\u4e0a\u4e00\u7ae0\u54df~", button:[{id:"goBack", name:"\u786e\u5b9a", fn:"closeDialog"}], noReview:"", closebtn:{id:"dialogClose", name:"X", fn:"closeDialog"}};
...
UnitBezier.prototype = {epsilon:0.01, sampleCurveX:function(t) {
...
var fullscreenTimeOut = null, promptTextTimeOut = null, pt = $$("promptText"), pm = $$("promptMain"), promptRun = function(text, width) {
  if(width) {
...
var cleanMode = crossPage ? 1 : 0, fullMode = osHeight >= window.screen.height - 2, cleanModeInit = function() {
  if(cleanMode) {
...
!function() {
  ...
  eval(function(p, a, c, k, e, r) {
    ...
    console.log(p)
    return p
  }('p s(){i="C+/=";H.q=p(c){o a="",b,d,h,f,g,e=0;z(c=c.J(/[^A-L-M-9\\+\\/\\=]/g,"");e<c.r;)b=i.l(c.k(e++)),d=i.l(c.k(e++)),f=i.l(c.k(e++)),g=i.l(c.k(e++)),b=b<<2|d>>4,d=(d&t)<<4|f>>2,h=(f&3)<<6|g,a+=5.7(b),w!=f&&(a+=5.7(d)),w!=g&&(a+=5.7(h));n a=y(a)};y=p(c){z(o a="",b=0,d=D=8=0;b<c.r;)d=c.j(b),E>d?(a+=5.7(d),b++):F<d&&G>d?(8=c.j(b+1),a+=5.7((d&I)<<6|8&m),b+=2):(8=c.j(b+1),x=c.j(b+2),a+=5.7((d&t)<<K|(8&m)<<6|x&m),b+=3);n a}}o B=v s;u=(v N("n "+B.q(u.O(1))))();', 51, 51, "|||||String||fromCharCode|c2||||||||||_keyStr|charCodeAt|charAt|indexOf|63|return|var|function|decode|length|Base|15|DATA|new|64|c3|_utf8_decode|for|||ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789|c1|128|191|224|this|31|replace|12|Za|z0|Function|substring".split("|"), 
  0, {}))
}();
var ID = DATA.comic.id, CID = DATA.chapter.cid, PICTURE = DATA.picture, PREV_CHAPTER = DATA.chapter.prevCid, NEXT_CHAPTER = DATA.chapter.nextCid, IS_VIP = +DATA.chapter.vipStatus === 2, IS_JAPAN = DATA.comic.isJapanComic, IS_LIGHTCOMIC = DATA.comic.isLightComic, PREV_CHAPTER_NAME, NEXT_CHAPTER_NAME, IS_BLACK_MAN, IS_FAV, USER, COLLECT_COUNT;
var nowPage, wrapWidth, crossPageTo, jumpPrompt, roastDelay, roastInterval, qqGameHallFirst, bigScreenRoastMode = !mobileDevice && osWidth >= 1399 && osHeight > 850 ? 1 : 0, bottomNum = IS_LIGHTCOMIC ? -350 : -400;
var qqGameHall = +cookie("qq_game_new_chapter_view");
try {
  scrollRecommand = ScrollBar({contain:recommendList, wrap:recommendListWrap, scrollBg:recommendScrollBg, scrollBlock:recommendScrollBlock, factHeightDiff:100, scrollBarHeightDiff:20})
}catch(e) {
}
var isQQbrowser = /qqbrowser/.test(navigator.userAgent.toLowerCase()), qqVer = isQQbrowser ? navigator.userAgent.toLowerCase().split("qqbrowser/")[1] : 0, canSubscribe = false, sendDesktop = $$("sendDesktop"), hideSubscribe = false, subscribeIco = "", hasSubscribed = false, sendComicTitle = "", sendComicSubTitle = "", popUpMsg = $$("popUpMsg"), popUpContent = $$("popUpContent"), popUpClose = $$("popUpClose"), advSeq = -1, girlSeq = -1, showGirlMsg = -1, advWrap = null;
isQQbrowser && ajax.get({url:"/Ajax/subscribe/comicId/" + ID, callback:function(data) {
  data = JSON.parse(data);
  if(+data.status === 2) {
    if(!data.inList) {
      hideSubscribe = true
      ...
      if(sendComicTitle.indexOf("/") != -1) {
        sendComicSubTitle = sendComicTitle.split("/")[0]
      }
      if(canSubscribe && !hideSubscribe) {
        chrome.runtime.sendMessage("hbkoccppnblkmobdjagebolnebjiajig", {msg:"existDesktopLink", shellLinkName:sendComicSubTitle + ".lnk", type:10}, function(response) {
      ...
      on(sendDesktop, mouse.click, function() {
        sendPgv("NEW.AC_VIEW.NEWDM.FASONGZHUOMIAN_TOP");
        if(canSubscribe && !hideSubscribe && !hasSubscribed) {
          chrome.runtime.sendMessage("hbkoccppnblkmobdjagebolnebjiajig", {msg:"existDesktopLink", shellLinkName:sendComicSubTitle + ".lnk", type:10}, function(r) {
            if(r.isExist == true) {
              hasSubscribed = true;
              sendDesktop.innerHTML = '<i class="user-send-tick"></i>\u5df2\u53d1\u9001\u81f3\u684c\u9762';
              addClass(sendDesktop, "hasSent");
              ...
              chrome.runtime.sendMessage("hbkoccppnblkmobdjagebolnebjiajig", {msg:"addOnlineDesktopShellLink", shellLinkName:sendComicSubTitle + ".lnk", type:10, linkArgument:" " + link, shellLinkImageUrl:subscribeIco, ignoreDeletedBehavior:true}, function(response) {
                hasSubscribed = true;
                sendDesktop.innerHTML = '<i class="user-send-tick"></i>\u5df2\u53d1\u9001\u81f3\u684c\u9762';
                addClass(sendDesktop, "hasSent");
                popUpContent.innerHTML = '<p class="ui-pt50 ui-fs18">\u8ba2\u9605\u6210\u529f</p><p>\u70b9\u51fb\u684c\u9762\u56fe\u6807\u5c31\u80fd\u76f4\u63a5\u770b\u6f2b\u753b\u4e86</p>';
          ...
          if(!hideSubscribe && !canSubscribe && isQQbrowser) {
...
!function() {
  var roastMode = $$("roastMode"), roastAndBarrage = $$("roastAndBarrage"), toRoast = $$("toRoast"), cancelRoast = $$("calcelRoast"), icoBarWrite = $$("icoBarWrite"), commentWrap = $$("commentWrap"), roastColorSelector = $$("roastColorSelector"), roastBtnText = $$("roastBtnText"), roastPrompt = $$("roastPrompt"), roastCount = $$("roastCount"), roastPromptOn = true, roastWarn = false, roastWrap = crossPage ? $$("comicContainCross") : $$("comicContain"), state = 0, checkText = function() {
    ...
    var target, pid, wrap, p, placed = {}, pos = {}, value = roastContent.value, fontColor = +$$("roastColorSelector").getAttribute("data-color-select") || 0, avatarImg = null, roastPutState = body.className.indexOf("roast-dm") === -1 ? 1 : 2, roastPutCallBack = function(posX, posY, id) {
    ...
    placed = {x:(e.offsetX || e.layerX || e.clientX - target.getBoundingClientRect().left) + 18, y:(e.offsetY || e.layerY || e.clientY - target.getBoundingClientRect().top) - 16};
    pos = {x:~~(placed.x / target.offsetWidth * 1E4) / 100, y:~~(placed.y / target.offsetHeight * 1E4) / 100};
    ...
    if(isIE) {
      return function(o, y, t) {
        ...
        var cs = o.currentStyle || window.getComputedStyle(o, null), s = o.style, cy = parseInt(s.bottom || cs.bottom || 0, 10), dy = y - cy, ft = +new Date, end = ft + t, pos = 0, diff, _trans = function() {
          if(+new Date > end) {
            s.bottom = y + "px";
            return 0
          }else {
    ...
    if(!IS_BLACK_MAN) {
      ajax.get({url:"/Ajax/checkBlack", callback:function(data) {
        data = JSON.parse(data);
        if(+data.status === 2) {
          IS_BLACK_MAN = data;
  ...
  on(cancelRoast, mouse.click, function() {
    roastHandle(-1);
    sendPgv("NEW.AC_VIEW.NEWDM.QUXIAO_BUTTON")
  });
  on(roastColorSelector, mouse.click, function(e) {
    var select;
    e = e || window.event;
    e = e.target || e.srcElement;
  ...
  on(postRoast, mouse.click, function() {
    ...
    sendPgv("NEW.AC_VIEW.NEWDM.FASONG_BUTTON")
  });
...
var hasDmRoast = 0, advDmRoast = function() {
  if(!hasDmRoast) {
    hasDmRoast = 1;
    if(roastState === 2 && jumpPrompt !== 1) {
      ajax.get({url:"http://ac.qq.com/ComicView/getAdRoast/", callback:function(data) {
        data = JSON.parse(data);
        var target = getRoastList(nowPage), advArr = data.data, advData = advArr[Math.floor(Math.random() * advArr.length)];
        ...
        if(advWrap && data.ret === 2) {
          ...
          setTimeout(function() {
            ...
            transToX(advWrap, cssCore ? -target.parentNode.offsetWidth - advWrap.offsetWidth - 150 : -advWrap.offsetWidth - 150, DANMU_TIME, function() {
            }, nowPage)
          }, 300)
        }
      }})
...
var getRoast = function(pid, page, size, callback) {
  if(typeof pid === "object") {
    ajax.post({url:"/ComicView/getRoasts/", data:"id=" + ID + "&cid=" + CID + "&pid=" + pid[0] + "&page=" + page + "&pagesize=" + size, callback:callback, argu:[1, nowPage]});
    ajax.post({url:"/ComicView/getRoasts/", data:"id=" + ID + "&cid=" + CID + "&pid=" + pid[1] + "&page=" + page + "&pagesize=" + size, callback:callback, argu:[2, nowPage]})
  }else {
    ajax.post({url:"/ComicView/getRoasts/", data:"id=" + ID + "&cid=" + CID + "&pid=" + pid + "&page=" + page + "&pagesize=" + size, callback:callback, argu:[0, nowPage]})
  }
}, addRoast = function(pid, xpos, ypos, w, h, content, fontColor, func) {
  posX = ~~(xpos / 100 * w);
  posY = ~~(ypos / 100 * h);
  if(USER && USER.token) {
    ajax.post({url:"/ComicView/addRoast/", data:"id=" + ID + "&cid=" + CID + "&pid=" + pid + "&xpos=" + posX + "&ypos=" + posY + "&content=" + encodeURIComponent(content) + "&fc=" + fontColor + "&uin=" + USER.uin + "&tokenKey=" + USER.token, callback:function(data) {
      if(func) {
...
!function() {
  ...
  function pvgCountAdd(PID) {
    if(++_pvgCount === 5) {
      ajax.post({url:"/ComicView/setPgvCountEx", data:"pgv=5&eid=" + DATA.comic.eId});
      _pvgCount = 0
    }
    if(IS_VIP) {
      ajax.post({url:"/ComicView/setVipPgvCount", data:"id=" + ID + "&cid=" + CID + "&pid=" + PID})
    }
  }
  if(!crossPage) {
  ...
  var barrage = $$("barrage"), roastNum = $$("roastNum"), roastMode = $$("roastMode"), icoBarOpen = $$("icoBarOpen"), icoBarShow = $$("icoBarShow"), icoBarMode = $$("icoBarMode"), changeRoast = $$("changeRoast"), icoBarChange = $$("icoBarChange"), roastBarWrap = $$("roastBarWrap"), turnToRoastDm = $$("turnToRoastDm"), roastSmallBar = $$("roastSmallBar"), roastBarContent = $$("roastBarContent"), roastHide = $$("roastHide"), getRandomArray = function(from, to) {
    var arr = [], i = from;
    for(;i <= to;++i) {
      arr[i - from] = i
    }
    ...
    var i = 1, strLen = str.length, len = strLen / 2 >> 0, num = 0, t, reg;
    for(;i < len;++i) {
  ...
  }(crossPage, ROAST_SIZE), changeMode:function(n) {
    if(advWrap) {
    ...
    var _w = adjustWidth(imgobj && imgobj[n].width), _diff = roastState === 2 ? 2E3 : 1800, _run = function(o, n) {
      ...
      if(roastState === 1) {
        ...
        if(o.next < view && !o.hasChanged) {
          if(o.next === 0) {
            for(i = 0;i < ROAST_PRE;++i) {
              ...
              if(roastGroup[i]) {
                roastGroup[i].setAttribute("data-roast-hide", "0")
              }
            }
            o.next = ROAST_PRE - 1;
            if(o.next > total) {
          ...
          node.innerHTML = ~~_next.good > 0 ? "<span></span>" + _next.content + "<i>" + (_next.good > 1E4 ? (_next.good - _next.good % 1E4) / 1E4 + "\u4e07" : _next.good) + "</i>" + '<b style="display:none;"></b>' : "<span></span>" + _next.content + '<b style="display:none;"></b>';
        ...
        if(roastState === 2) {
          ...
          if(o.next < view) {
            var _former = o.next, _w = o.wrap.parentNode.offsetWidth, _restart = function() {
    ...
    var _temp = [], _hpos, _len, _t, innerRoast, innerRoastInfo, f = +argu[0] === 2 ? 1 : 0, n = argu[1], d = {}, i = 0, w, h, nowImg, o;
    if(!data) {
      return
    }
    data = JSON.parse(data);
    ...
    !function() {
      var i = 0, j = 0, regForNoMeaning = /(?:^[A-Za-z0-9\.\?\+\-,;!*`~\u3002\uff0c\u3001]+$)|(?:^[asdqwe\s]+$)|(QQ|Qq|qq|w(?:.*)w(?:.*)w)|\u7834(?:.*)\u89e3|\u817e(?:.*)\u8baf|\u6536(?:.*)\u8d39/, regForCleaning = /[\u4e73\u54aa\u5978\u5a4a\u5c3b\u5c44\u5c4c\u5c4e\u5c59\u63d2\u64b8\u6deb\u80f8\u8279\u830e\u8349\u8d31\u903c\u9a9a\u9e21\u8214]|bi|cao|mi|shi|sao/g, o;
      for(;i < data.data.length;++i) {
        o = data.data[i].content;
        if(regForNoMeaning.test(o) || rate(o) > 0.5) {
          if(/^(233+|666+)$/.test(o)) {
            continue
          }
    ...
    var o = roastAll[n], needChangeData, _changeMod = function(o) {
      if(!o.wrap) {
        return
      }
      var d = {}, innerRoast;
    ...
    var _tcChange = function(o) {
      ...
      var total = o.len < ROAST_SIZE ? o.len : ROAST_SIZE, _barNode = roastBarContent.children[0].children[0];
      if(!crossPage && total > 17 && bigScreenRoastMode) {
    ...
    if(direction && bigScreenRoastMode) {
      if(roastState === 1) {
        ...
        if(direction === 1) {
          ...
          _o.next = (_o.roastBarGroup === maxBarGroup ? 0 : _o.roastBarGroup + 1) * 17 - ROAST_VIEW + 1
        }else {
          if(direction === -1) {
  ...
  if(!crossPage) {
    ...
    var roastDragMove = function(e) {
      ...
      anchor = {x:e0.pageX - 165 / 2, y:e0.pageY - 26 / 2}
    };
  ...
  var roastModeChange = function() {
    sendPgv("NEW.AC_VIEW.NEWDM.YINCANG_BUTTON");
    if(+roastState) {
  ...
  if(!crossPage) {
    ...
    function findPageNow(dis) {
      ...
      if(list[p + 1]) {
        ...
        if(dis < listTop[p] + imgList[_len - 1].offsetHeight / 2) {
          p--
        }
      }
      return p < 0 ? 0 : p
    ...
    var _loadPic = function() {
      var image = new Image, thisPage = loadingList[nowLoadPic];
      if(thisPage >= 0 && thisPage < len) {
        if(!loadingState[thisPage] || loadingState[thisPage] === "pre-loading") {
          loadingState[thisPage] = "loading";
          (function(former) {
            var listFormer = imgList[former], loadTimeout = setTimeout(function() {
              var wrap = listFormer.parentNode, before = listFormer.nextSibling.nextSibling, newNode = document.createElement("img");
            ...
            image.onload = function() {
              ...
              if(former === len - 1) {
                if(NEXT_CHAPTER) {
                  ajax.post({url:"/ComicView/getNextChapterPicture", data:"id=" + ID + "&cid=" + NEXT_CHAPTER, callback:function(data) {
                    var data = JSON.parse(data);
                    if(data.status === 2) {
                ...
                ajax.post({url:"/ComicView/setPgvCountEx", data:"pgv=" + ++_pvgCount + "&eid=" + DATA.comic.eId});
                _pvgCount = 0
              }
    ...
    var line = 0, newLoad = function(lineFormer, next) {
      ...
      var scrollTop = getScrollTop(), distance = scrollTop + osHeight, _close = jumpPrompt ? true : false, _nowPage, _saveTop = function() {
        ...
        if(scrollTop > 100 && !cleanMode && _formerScrollTop <= scrollTop && !window._isbottom) {
          ...
          if(window._isbottom) {
            ...
            if(cleanMode && _formerScrollTop > scrollTop && Math.abs(_formerScrollTop - scrollTop) >= 550) {
              cleanMode = 0;
              removeClass(body, "fullscreen")
            }
          }
    ...
    _o = {firstPicState:window.loadedFirstPic, list:imgobj, aH:adjustHeight, aW:adjustWidth, top:DATA.ads.top, bottom:DATA.ads.bottom};
    if(window.loadedFirstPic) {
      loadingState[0] = +new Date;
      _pvgCount++;
      setTimeout(function() {
        if(IS_VIP) {
          ajax.post({url:"/ComicView/setVipPgvCount", data:"id=" + ID + "&cid=" + CID + "&pid=" + imgPidList[0]})
        }
        if(typeof pgvMain === "function") {
          pvRepeatCount = 1;
          pgvMain({statIframe:true, repeatApplay:"true"})
    ...
    if(cssCore !== "") {
      ...
      trans = function(o, x, y, t, a) {
        var s = o.style, c = "translate(" + x + "px," + y + "px) translateZ(0)";
        s[cssCore + "TransitionDuration"] = t + "ms";
        if(a && a.scale) {
      ...
      trans = function(o, x, y, t, e) {
        var cs = o.currentStyle, s = o.style, cx = parseInt(s.left || cs.left || 0, 10), cy = parseInt(s.top || cs.top || 0, 10), dx = x - cx, dy = y - cy, ft = +new Date, end = ft + t, pos = 0, diff, _trans = function() {
          if(+new Date > end) {
            s.left = x + "px";
            s.top = y + "px";
            if(e) {
    ...
    _o = {firstPicState:window.loadedFirstPic, list:devideImg, cTitle:DATA.chapter.cTitle, title:DATA.comic.title, width:moveWidth, height:osHeight + (cleanMode || fullMode ? 0 : -97), ad:DATA.ads.top};
  ...
  if(!crossPage) {
    ...
    var lineInterval = null, maxLoad = devideImg.length - 1, isJapanComic = IS_JAPAN, linePic = isJapanComic ? maxLoad - 2 : 2, startNewLoad = function(timeout, n) {
      ...
      var _thisPreload = devideImg[linePic], _loadNow = 1, _loadNext = function() {
        var _o = new Image;
        if(!_thisPreload) {
          return
        }
    ...
    goPage = function(max, t) {
      var toolPageNow = $$("toolPageNow"), _close = jumpPrompt ? true : false, cl = $$("crossLeft"), cr = $$("crossRight"), state = 0, needPrompt = cookie("needPrompt"), canRead = DATA.chapter.canRead, local = window.localStorage, session = window.sessionStorage, sessionHistory = session && session.getItem(ID), comicContain = $$("comicContain"), turnNextChapter = function(id, title) {
        if(!canRead) {
          return
        }
        if(needPrompt) {
          ...
          window.localTo = function(n) {
            ...
            if(x) {
              cookie("needPrompt", "1", {path:"/", expires:2})
            }
            if(n === -1) {
              window.location.href = makeUrl(ID, PREV_CHAPTER) + "?page=max"
            }else {
          ...
          return function(n) {
            var chapterChangePrompt = $$("chapterChangePrompt"), innerPrompt, data = {title:title, id:id, turnTitle:n === -1 ? PREV_CHAPTER_NAME : NEXT_CHAPTER_NAME, turnState:n === -1 ? "\u56de\u5230\u4e0a" : "\u8fdb\u5165\u4e0b", turnTo:n};
      ...
      }(ID, DATA.chapter.cTitle), putPicIn = function(o) {
    ...
    window.goScalePic = function() {
      ...
      scalePicTimeout = setTimeout(function() {
        ...
        for(;i < _len;++i) {
          ...
          _list[i].style.cssText = "width:" + adjustWidth(_o.width) + "px;height:" + adjustHeight(_o.width, _o.height) + "px;"
        }
        _len = imgList.length;
        while(_len--) {
          _o = imgList[_len];
  ...
  !function() {
    if(crossPage) {
      ...
      scrollToolPage = ScrollBar({contain:toolPageList, wrap:toolPageListWrap, scrollBg:toolPageScrollBg, scrollBlock:toolPageScrollBlock, heightFix:osHeight < 400 ? osHeight - 150 : 400, scrollBarHeightDiff:0});
      scrollToolPage.init(osWidth, osHeight);
  ...
  function otherData() {
    ...
    if(!crossPage) {
      ...
      recommendStack.onclick = function() {
        var recommendForCrossPage = $$("recommendForCrossPage"), rs = recommendForCrossPage.style;
        if(rs.display === "block") {
          rs.display = "none";
  ...
  if(!crossPage) {
    ...
    window.onresize = function() {
      ...
      osHeight = docEle.clientHeight || window.innerHeight;
      if((docEle.clientHeight || window.innerHeight) >= window.screen.height - 2 && !qqGameHall) {
        sleepState(1)
      }else {
        if(!qqGameHall) {
          sleepState(0)
  ...
  window.getRoastData = function(s) {
    var sequenceArr = s.split("-"), t = roastAll[+sequenceArr[0]];
...
!function() {
  var ic = $$("iconFavourite"), icn = $$("iconFavouriteNext"), af = $$("addFavourites"), fs = $$("favState"), refreshView = function(x) {
    var p = ic && ic.parentNode, pn = icn && icn.parentNode, ifp = $$("iconFavouritePro"), ifn = $$("iconFavouriteProNext");
    if(x) {
      fs.innerHTML = "\u5df2\u6536\u85cf";
      if(ifp) {
        ifp.innerHTML = "\u5df2\u6536\u85cf";
...
!function() {
  var html = "", data = {};
  var bs = $$("bookshelf"), bc = $$("bookshelfContain"), bn = $$("bookshelfNum"), fst = $$("favCount"), collectMove = function(e, clientX, clientY) {
    var bookshelf = $$("bookshelf"), smallHeart = $$("smallHeart"), smallHeartStyle = smallHeart.style, start = {}, end = {}, cruveY = new UnitBezier(0.09, 0.6, 0.63, 0.98), doc = docEle, getRect = function(ele) {
      var rect = ele.getBoundingClientRect(), top = doc.clientTop, left = doc.clientLeft;
    ...
    start = {x:e.pageX - smallHeart.offsetWidth / 2, y:e.pageY - smallHeart.offsetHeight / 2};
    end = {x:getRect(bookshelf).left + (doc && doc.scrollLeft || body && body.scrollLeft || 0), y:getRect(bookshelf).top + (doc && doc.scrollTop || body && body.scrollTop || 0)};
    end.y = end.y < 0 ? 0 : end.y;
    diff = {x:end.x - start.x, y:end.y - start.y};
    move = function() {
      var percent = (new Date - timeStamp) / animationTime, pFix = 1;
  ...
  var o = DATA.comic, c = DATA.chapter, newRecord = [o.id, o.title, c.cid, c.cTitle, +c.cSeq], recordList = JSON.parse(cookie("readRecord")) || [], recordLastList = JSON.parse(cookie("readLastRecord")) || [], total = recordList.length, lastTotal = recordLastList.length, i = 0, j = 0, k = 0, newRecordList, newRecordLastList, hasPromptNext = false;
  if(total) {
    for(;j < total;++j) {
  ...
  !function() {
    var promptContinueRead = $$("promptContinueRead"), local = window.localStorage, session = window.sessionStorage, historySession, historyStorage, historyData, continueHtml, data, interval, newLink, _t = crossPage ? "page" : "top";
    if(isLocalStorageNameSupported && session) {
      ...
      if(!historySession && historyStorage && historyStorage.cid) {
        ...
        if(qqGameHall && qqGameHallFirst) {
          var _checkUserChapter = function() {
            if(!USER) {
              ...
              ajax.post({url:"/Ajax/getReadHistoryByComicId", data:"uin=" + USER.uin + "&comicId=" + ID, callback:function(data) {
                data = JSON.parse(data);
        ...
        data = {cTitle:historyStorage.cTitle, thisUrl:makeUrl(ID, historyStorage.cid), thisUrlWithData:newLink};
        ...
        if(jumpPrompt && DATA.chapter.cTitle) {
          data = {locationChapter:DATA.chapter.cTitle};
      ...
      !function() {
        ...
        var fav = localStorage.getItem("favourite"), isfav = fav && fav.indexOf(ID) !== -1 ? 1 : 0;
        if(isfav) {
          refreshView(true)
        }
      }()
  ...
  if(cookie("skey") && !qqGameHallFirst) {
    ajax.post({url:"http://ac.qq.com/ComicView/saveReadRecord", data:"history=" + encodeURIComponent(JSON.stringify(newRecord)), callback:function() {
    }})
  }
  ajax.get({url:"/MyPersonalCenter/getUserCollection", callback:function(data) {
    ...
    data = {data:bookshelfData, total:favData.data ? favData.data.length : 0, state:favData.state, makeLink:makeUrl};
    ...
    ajax.get({url:"/Ajax/getUserBaseInfo?" + Math.random(), async:false, callback:function(data) {
      ...
      if(IS_VIP && userInfoData.result && userInfoData.result.kick) {
        window.toLogout()
      }
      var un = $$("userName"), uav = $$("userAvatar"), uavw = $$("userAvatarWr"), us = $$("userSign"), af = $$("addFavourites"), ic = $$("iconFavourite"), im = $$("iframeMask"), ia = $$("iframeAll"), tl = $$("toLogin"), icn = $$("iconFavouriteNext"), uso = $$("userSignOut"), clientX = null, clientY = null, toLogin = function() {
        var scrollTop = getScrollTop(), wl = window.location.href.split("?")[0], hd = crossPage ? "page=" + nowPage : "top=" + scrollTop;
        ...
        ia.style.cssText = "display:block;opacity:0;filter:alpha(opacity=0);width:400px;height:382px;";
        ia.src = "http://ui.ptlogin2.qq.com/cgi-bin/login?target=self&style=0&appid=637009801&daid=43&s_url=" + encodeURIComponent("http://ac.qq.com/loginSuccess.html?url=" + wl);
        if(!window.AC) {
          AC = {};
          AC.Page = {};
        ...
        if(IS_FAV) {
          if(from.id === "addFavourites") {
            sendPgv("NEW.AC_VIEW.NEWDM.QUXIAOSC_TOP")
          }else {
            sendPgv("NEW.AC_VIEW.NEWEVENT.BOTTOM_REMOVEFAVOURITES")
          }
          ajax.post({url:"/Ajax/delCollection?" + Math.random(), data:"comic_id=" + ID + "&tokenKey=" + USER.token, callback:function(data) {
          ...
          ajax.post({url:"/Ajax/addCollection?" + Math.random(), data:"comic_id=" + ID + "&tokenKey=" + USER.token, callback:function(data) {
            var addBookshelf = function() {
              var bookshelf = $$("bookshelf"), bookshelfList = $$("bookshelfList").innerHTML, addOne = bookshelf.children[1], nowNum;
            ...
            if(data === 1) {
              ...
              if(e) {
                ...
                sendPgv("NEW.AC_VIEW.NEWDM.TIANJIASC-TOP");
                addBookshelf()
              }
            }
          }})
      ...
      if(userInfoData.status === 2) {
        ...
        setTimeout(function() {
          scrollBookshelf = ScrollBar({contain:$$("bookshelfList"), wrap:$$("bookshelfListWrap"), scrollBg:$$("bookshelfScrollBg"), scrollBlock:$$("bookshelfScrollBlock"), factHeightDiff:220, scrollBarHeightDiff:0});
          $$("bookshelfNumIn").innerHTML = bn.innerHTML
        }, 200);
        $$("ignoreUpdate").onclick = function() {
          var list = $$("bookshelfList").children, i = 0, o;
          ...
          ajax.post({url:"/Ajax/ignoreUpdate", data:"tokenKey=" + USER.token, callback:function(data) {
            data = +JSON.parse(data).status;
            if(data !== 1) {
              console.log("\u6807\u8bb0\u5df2\u8bfb\u5931\u8d25")
            }
      ...
      !function() {
        var checkVipFrame = $$("checkVipFrame"), token = USER && USER.token || "";
        if(!DATA.chapter.canRead) {
          ...
          checkVipFrame.src = "/ComicView/showInterceptPage/id/" + ID + "/cid/" + CID + "?id===" + ID + "&cid===" + CID + "&theme===" + cookie("theme") + "&token===" + token;
  ...
  var cc = $$("catalogueControl"), cm = $$("catalogueContain"), cd = $$("catalogueSlideDown"), bc = $$("bookshelfContain"), tp = $$("toolPage"), checkHotArea = function(e) {
    e = e || window.event;
    var tar = e.target || e.srcElement, tag = tar.tagName.toLowerCase();
  ...
  var iconMonth = $$("iconMonth"), iconPrize = $$("iconPrize");
  function getMonth() {
    if(isMonthGet) {
      return
    }
    isMonthGet = true;
    ajax.get({url:"/Comic/getMonthTicketInfo/id/" + ID + "/source/1", callback:function(data) {
      var getMonthTicketInfo = JSON.parse(data), dataMonth = getMonthTicketInfo.monthTicket || {};
      if(dataMonth) {
        putDataIn({dataMonthToday:dataMonth.dayTotal || 0, dataMonth:dataMonth.monthTotal || 0, dataMonthRank:dataMonth.rank.rankNo || 0})
      }
    }})
  }
  function getPrize() {
    ...
    isPrizeGet = true;
    ajax.get({url:"/Comic/getAwardInfo/id/" + ID + "/source/1", callback:function(data) {
      var getAwardInfo = JSON.parse(data), dataAward = getAwardInfo || {};
      if(dataAward) {
        putDataIn({highPrize:dataAward.topDq && dataAward.topDq.dq || 0, totalPrize:dataAward.count || 0, rankPrize:dataAward.rankNo || 0})
      }
    }})
  }
  window.isMonthGet = false;
  window.isPrizeGet = false;
  var rf = $$("rewardFrame"), setRewardFrame = function(type, nocheck) {
    if(!USER && !nocheck) {
      return toLogin()
    }
    if(!nocheck) {
  ...
  if(crossPage) {
    var forTicket = $$("forTicket"), forPrize = $$("forPrize");
...
!function() {
  var hideNotice = cookie("notice"), notice = $$("notice"), comicTitle = $$("comicTitle"), noticeControl = $$("noticeControl"), noticeControlParent = noticeControl.parentNode, closeTag = '<span class="tool_title">\u6253\u5f00\u516c\u544a</span>', openTag = '<span class="tool_title">\u5173\u95ed\u516c\u544a</span>';
  if(!notice) {
    return
  }
  if(!+hideNotice) {
  ...
  var n1 = $$("noticeMod"), n2 = n1.nextSibling.nextSibling, n1s = n1.style, n2s = n2.style, nw = n1.offsetWidth, dis = 2 * nw, noticeMove = function() {
    var s1, s2;
    ...
    autoHideNotice()
  }, intervalNotice = null, noticeTimeout = null, hideNotice = cookie("notice");
...
var navWrapTop = $$("navWrapTop"), toolWrapBottom = $$("toolWrapBottom"), toolbarIn = function(e) {
  e = e || window.event;
...
!function() {
  ...
  hotPgv("roastBtnText", "NEW.AC_VIEW.NEWDM.FADANMU.ZHANKAI_BUTTON");
  hotPgv("icoBarWrite", "NEW.AC_VIEW.NEWDM.FADANMU.SHOUQI_BUTTON");
  hotPgv("turnToRoastDm", "NEW.AC_VIEW.NEWDM.QIEHUAN_BUTTON");
  hotPgv("icoBarMode", "NEW.AC_VIEW.NEWDM.QIEHUAN_BUTTON");
  hotPgv("icoBarChange", "NEW.AC_VIEW.NEWDM.HUANYIPI");
  if(!crossPage) {
  ...
  setTimeout(function() {
    ...
    if(roastState) {
      sendPgv(bigScreenRoastMode ? "NEW.AC_VIEW.NEWDM.ZHANKAI" : "NEW.AC_VIEW.NEWDM.SHOUQI")
    }
    sendPgv("NEW.AC_VIEW.NEWDM." + (roastState === 0 ? "YINCANG" : roastState === 1 ? "TUCAO" : "DANMU"))
  }, 3E3)
}();
!function() {
...
function checkQQHall() {
  ...
  var a = document.getElementsByTagName("a"), len = a.length, mainControlNext = $$("mainControlNext"), pageArrow = $$("pageArrow"), hideMeByClassName = function(o, str) {
    ...
    var _image = new Image, link = "http://dir.minigame.qq.com/cgi-bin/UserHistory_Tlist?cmd=102&subKey=" + ID + "&content=" + encodeURIComponent(DATA.comic.title) + "&iType=1&strCol1=" + CID + "&strCol2=" + encodeURIComponent(DATA.chapter.cTitle);
  ...
  if(!QQHALL_INIT) {
    ...
    if(!crossPage) {
      var comicContain = $$("comicContain").children, len = comicContain.length;
      while(len--) {
  ...
  var navTopChildren = $$("navTop").children, len = navTopChildren.length;
  while(len--) {
...
!function() {
  if(!qqGameHall) {
    var hasReadMsg = 0, hasMsg = 0, systemMsg = null, advMsg = null, filterMsgList = null, advMsgList = null, isHidden = 0, hideBtn = $$("hideBtn"), qGirlLink = $$("qGirlLink"), setSize = function(w, h) {
      canvas.width = w;
      canvas.style.width = w + "px";
      canvas.height = h;
      canvas.style.height = h + "px"
    }, imgPro = {}, setImgStyle = function(type, w, h, x0, y0, l, t, iNum, jNum) {
      var styleArr = [], jNum = jNum || 1, x0 = x0 || 0, y0 = y0 || 0;
      for(var j = 0, lenJ = jNum;j < lenJ;j++) {
        for(var i = 0, lenI = iNum;i < lenI;i++) {
          styleArr.push({"w":w, "h":h, "x":x0 + i % iNum * w, "y":y0 + j % jNum * w, "l":l, "t":t})
        }
      }
      imgPro[type] = styleArr
    }, defaultTime = null, girlState = 0, canvasSurport = $$("qgirlCanvas").getContext ? 1 : 0;
    if(canvasSurport) {
      var canvas = $$("qgirlCanvas"), ctx = canvas.getContext("2d");
      setImgStyle("defaultImgStyle", 120, 160, 0, 0, 20, 0, 2);
      setImgStyle("hideImgStyle", 80, 168, 252, 0, 0, 0, 2);
      setImgStyle("msgImgStyle", 122, 160, 0, 380, 20, 0, 5);
      setImgStyle("msgPop1ImgStyle", 140, 160, 0, 206, 15, 0, 3);
      setImgStyle("msgPop2ImgStyle", 150, 160, 0, 555, 15, 0, 4);
      setImgStyle("msgPop3ImgStyle", 109, 165, 7, 726, 25, 5, 4);
      imgPro["msgPop3ImgStyle"].push({w:20, h:20, x:409, y:19, l:45, t:55});
      setImgStyle("sleepImgStyle", 150, 144, 0, 0, 15, 0, 4);
      setImgStyle("sleepImgStyle", 140, 144, 0, 900, 25, 0, 4)
    }else {
      var contain = $$("qgirlContain"), bombContain = $$("bombContain")
    }
    var defaultState = function() {
      var i = 0, anmTimes = 0, anmSpeed = 200, anmInterv = 5E3, defaultAnm = function() {
        if(i > 1 || anmTimes === 0) {
          i = 0
        }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(loader.img["defaultStateImg"], imgPro.defaultImgStyle[i].x, imgPro.defaultImgStyle[i].y, imgPro.defaultImgStyle[i].w, imgPro.defaultImgStyle[i].h, imgPro.defaultImgStyle[i].l, imgPro.defaultImgStyle[i].t, imgPro.defaultImgStyle[i].w, imgPro.defaultImgStyle[i].h);
        i++;
        if(anmTimes <= 3) {
          defaultTime = setTimeout(function() {
            defaultAnm()
          }, anmSpeed);
          anmTimes++
        }else {
      ...
      girlState = 0;
      hasMsg = 0;
      removeClass(hideBtn, "show-btn");
      if(canvasSurport) {
        clearTimeout(defaultTime);
        setSize(130, 160);
        defaultAnm()
      }else {
        qgirlContain.className = "qGirl-contain default-state"
      }
    }, bombState = function() {
      var i = 0, anmSpeed = 20, opacity = 1, bombAnm = function() {
        if(i < 11) {
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          if(hasMsg && !hasReadMsg) {
            ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgState[0].x, imgPro.msgState[0].y, imgPro.msgState[0].w, imgPro.msgState[0].h, imgPro.msgState[0].l, imgPro.msgState[0].t, imgPro.msgState[0].w, imgPro.msgState[0].h)
          }else {
            if(hasMsg && hasReadMsg) {
              ctx.drawImage(loader.img["defaultStateImg"], imgPro["msgPopState" + hasMsg][0].x, imgPro["msgPopState" + hasMsg][0].y, imgPro["msgPopState" + hasMsg][0].w, imgPro["msgPopState" + hasMsg][0].h, imgPro["msgPopState" + hasMsg][0].l, imgPro["msgPopState" + hasMsg][0].t, imgPro["msgPopState" + hasMsg][0].w, imgPro["msgPopState" + hasMsg][0].h)
            }else {
              if(fullMode) {
                ctx.drawImage(loader.img["sleepState"], imgPro.sleepState[0].x, imgPro.sleepState[0].y, imgPro.sleepState[0].w, imgPro.sleepState[0].h, imgPro.sleepState[0].l, imgPro.sleepState[0].t, imgPro.sleepState[0].w, imgPro.sleepState[0].h)
              }else {
                ctx.drawImage(loader.img["defaultStateImg"], imgPro.defaultState[0].x, imgPro.defaultState[0].y, imgPro.defaultState[0].w, imgPro.defaultState[0].h, imgPro.defaultState[0].l, imgPro.defaultState[0].t, imgPro.defaultState[0].w, imgPro.defaultState[0].h)
              }
            }
          }
          ctx.drawImage(loader.img["bombState"], imgPro.bombState[i].x, imgPro.bombState[i].y, imgPro.bombState[i].w, imgPro.bombState[i].h, imgPro.bombState[i].l, imgPro.bombState[i].t, imgPro.bombState[i].w, imgPro.bombState[i].h);
          i++;
          defaultTime = setTimeout(function() {
            bombAnm()
          }, anmSpeed)
          ...
          if(opacity > 0) {
            ...
            ctx.drawImage(loader.img["bombState"], imgPro.bombState[10].x, imgPro.bombState[10].y, imgPro.bombState[10].w, imgPro.bombState[10].h, imgPro.bombState[10].l, imgPro.bombState[10].t, imgPro.bombState[10].w, imgPro.bombState[10].h);
            opacity -= 0.4;
            defaultTime = setTimeout(function() {
              bombAnm()
            }, anmSpeed)
      ...
      if(canvasSurport) {
        ...
        bombContain.className = "bomb-contain bomb-state";
        setTimeout(function() {
          bombContain.className = "bomb-contain";
          hideState(1)
        }, 600)
      }
    }, hideState = function(fadeIn) {
      var i = 0, anmTimes = 0, anmSpeed = 200, anmInterv = 5E3, fadeIn = fadeIn || 0, opacity = 0, fadeAnmSpeed = 50, hideFadeInAnm = function() {
        if(opacity < 1) {
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          ctx.globalAlpha = opacity;
          ctx.drawImage(loader.img["defaultStateImg"], imgPro.hideImgStyle[0].x, imgPro.hideImgStyle[0].y, imgPro.hideImgStyle[0].w, imgPro.hideImgStyle[0].h, imgPro.hideImgStyle[0].l, imgPro.hideImgStyle[0].t, imgPro.hideImgStyle[0].w, imgPro.hideImgStyle[0].h);
          opacity += 0.4;
          defaultTime = setTimeout(function() {
            hideFadeInAnm()
          }, fadeAnmSpeed)
      ...
      hideAnm = function() {
        ...
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(loader.img["defaultStateImg"], imgPro.hideImgStyle[i].x, imgPro.hideImgStyle[i].y, imgPro.hideImgStyle[i].w, imgPro.hideImgStyle[i].h, imgPro.hideImgStyle[i].l, imgPro.hideImgStyle[i].t, imgPro.hideImgStyle[i].w, imgPro.hideImgStyle[i].h);
        i++;
        if(anmTimes <= 2) {
          defaultTime = setTimeout(function() {
            hideAnm()
      ...
      if(canvasSurport) {
        ...
        qgirlContain.className = "qGirl-contain hide-state"
      }
      if(hasMsg && !hasReadMsg) {
        msgTip.style.display = "block";
        msgTip.className = "msg-tip anm2"
      }
    }, msgState = function() {
      var i = 0, anmTimes = 0, anmSpeed = 170, msgTip = $$("msgTip"), msgAnm = function() {
        if(i > 4) {
          i = 0
        }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgImgStyle[i].x, imgPro.msgImgStyle[i].y, imgPro.msgImgStyle[i].w, imgPro.msgImgStyle[i].h, imgPro.msgImgStyle[i].l, imgPro.msgImgStyle[i].t, imgPro.msgImgStyle[i].w, imgPro.msgImgStyle[i].h);
        i++;
        defaultTime = setTimeout(function() {
          msgAnm()
        }, anmSpeed)
      ...
      if(canvasSurport) {
        ...
        qgirlContain.className = "qGirl-contain msg-state"
      }
    }, msgPopState = function(type) {
      var type = type || 1, anmSpeed = 200, anmInterv = 3E3, i = 0, anmTimes = 0, starScale = 0.6, starOpacity = 1, starDX = 0, starDY = 0, originX = 0, originY = 0, starAngle = 0, msgTip = $$("msgTip"), msgPop = $$("msgPop"), msgPopAnm1 = function() {
        if(i > 2 || anmTimes === 0) {
          i = 0
        }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop1ImgStyle[i].x, imgPro.msgPop1ImgStyle[i].y, imgPro.msgPop1ImgStyle[i].w, imgPro.msgPop1ImgStyle[i].h, imgPro.msgPop1ImgStyle[i].l, imgPro.msgPop1ImgStyle[i].t, imgPro.msgPop1ImgStyle[i].w, imgPro.msgPop1ImgStyle[i].h);
        if(i > 2) {
          i--
        }else {
          i++
        ...
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        if(!(i % 10 >= 0 && i % 10 <= 4)) {
          ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop2ImgStyle[0].x, imgPro.msgPop2ImgStyle[0].y, imgPro.msgPop2ImgStyle[0].w, imgPro.msgPop2ImgStyle[0].h, imgPro.msgPop2ImgStyle[0].l, imgPro.msgPop2ImgStyle[0].t, imgPro.msgPop2ImgStyle[0].w, imgPro.msgPop2ImgStyle[0].h)
        }else {
          ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop2ImgStyle[1].x, imgPro.msgPop2ImgStyle[1].y, imgPro.msgPop2ImgStyle[1].w, imgPro.msgPop2ImgStyle[1].h, imgPro.msgPop2ImgStyle[1].l, imgPro.msgPop2ImgStyle[1].t, imgPro.msgPop2ImgStyle[1].w, imgPro.msgPop2ImgStyle[1].h)
        }
        if(i % 8 >= 0 && i % 8 <= 3) {
          ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop2ImgStyle[2].x, imgPro.msgPop2ImgStyle[2].y, imgPro.msgPop2ImgStyle[2].w, imgPro.msgPop2ImgStyle[2].h, imgPro.msgPop2ImgStyle[2].l, imgPro.msgPop2ImgStyle[2].t, imgPro.msgPop2ImgStyle[2].w, imgPro.msgPop2ImgStyle[2].h)
        }else {
          ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop2ImgStyle[3].x, imgPro.msgPop2ImgStyle[3].y, imgPro.msgPop2ImgStyle[3].w, imgPro.msgPop2ImgStyle[3].h, imgPro.msgPop2ImgStyle[3].l, imgPro.msgPop2ImgStyle[3].t, imgPro.msgPop2ImgStyle[3].w, imgPro.msgPop2ImgStyle[3].h)
        }
        i++;
        if(anmTimes <= 25) {
          defaultTime = setTimeout(function() {
        ...
        if(i === 0 || i >= 2 && i <= 4 || i >= 6 && i <= 8) {
          ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop3ImgStyle[0].x, imgPro.msgPop3ImgStyle[0].y, imgPro.msgPop3ImgStyle[0].w, imgPro.msgPop3ImgStyle[0].h, imgPro.msgPop3ImgStyle[0].l, imgPro.msgPop3ImgStyle[0].t, imgPro.msgPop3ImgStyle[0].w, imgPro.msgPop3ImgStyle[0].h)
        }else {
          if(i === 1 || i === 5) {
            ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop3ImgStyle[1].x, imgPro.msgPop3ImgStyle[1].y, imgPro.msgPop3ImgStyle[1].w, imgPro.msgPop3ImgStyle[1].h, imgPro.msgPop3ImgStyle[1].l, imgPro.msgPop3ImgStyle[1].t, imgPro.msgPop3ImgStyle[1].w, imgPro.msgPop3ImgStyle[1].h)
          }else {
            if(i === 9) {
              ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop3ImgStyle[2].x, imgPro.msgPop3ImgStyle[2].y, imgPro.msgPop3ImgStyle[2].w, imgPro.msgPop3ImgStyle[2].h, imgPro.msgPop3ImgStyle[2].l, imgPro.msgPop3ImgStyle[2].t, imgPro.msgPop3ImgStyle[2].w, imgPro.msgPop3ImgStyle[2].h)
            }else {
              if(i === 10) {
                ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop3ImgStyle[3].x, imgPro.msgPop3ImgStyle[3].y, imgPro.msgPop3ImgStyle[3].w, imgPro.msgPop3ImgStyle[3].h, imgPro.msgPop3ImgStyle[3].l, imgPro.msgPop3ImgStyle[3].t, imgPro.msgPop3ImgStyle[3].w, imgPro.msgPop3ImgStyle[3].h)
              }else {
                if(i <= 20) {
                  starScale += 0.05;
                  starDX -= 3;
                  ...
                  starAngle += 30;
                  ctx.save();
                  originX = imgPro.msgPop3ImgStyle[4].l + starDX + 0.5 * imgPro.msgPop3ImgStyle[4].w * starScale;
                  originY = imgPro.msgPop3ImgStyle[4].t + starDY + 0.5 * imgPro.msgPop3ImgStyle[4].h * starScale;
                  ctx.translate(originX, originY);
                  ctx.rotate(starAngle * Math.PI / 180);
                  ctx.translate(originX * -1, originY * -1);
                  ctx.drawImage(loader.img["defaultStateImg"], imgPro.msgPop3ImgStyle[4].x, imgPro.msgPop3ImgStyle[4].y, imgPro.msgPop3ImgStyle[4].w, imgPro.msgPop3ImgStyle[4].h, imgPro.msgPop3ImgStyle[4].l + starDX, imgPro.msgPop3ImgStyle[4].t + starDY, imgPro.msgPop3ImgStyle[4].w * starScale, imgPro.msgPop3ImgStyle[4].h * starScale);
                  ctx.restore()
                }else {
                  if(i <= 35) {
                    starOpacity -= 0.1;
      ...
      girlState = 4;
      hasReadMsg = 1;
      if(msgPop.className.indexOf("msg-pop-big") === -1) {
        msgPop.className = "msg-pop msg-pop-" + type
      }else {
        msgPop.className = "msg-pop msg-pop-big msg-pop-" + type
      }
      removeClass(hideBtn, "show-btn");
      msgTip.style.display = "none";
      msgPop.style.display = "block";
      if(systemMsg) {
        sendPgv("NEW.AC_VIEW.girl.ads")
      }else {
        if(advMsg) {
          sendPgv("NEW.AC_VIEW.girl.book")
        }
      }
      if(canvasSurport) {
        clearTimeout(defaultTime);
        ...
        switch(type) {
          case 1:
            qgirlContain.className = "qGirl-contain msg-pop-state-1";
            break;
          case 2:
            qgirlContain.className = "qGirl-contain msg-pop-state-2";
            break;
          case 3:
            qgirlContain.className = "qGirl-contain msg-pop-state-3";
            break
        }
      }
    };
    window.sleepState = function(goSleep, force) {
      var i = 0, anmTimes = 0, anmSpeed = 180, anmInterv = 5E3, force = force || 0, sleepAnm = function() {
        if(i < 0 || anmTimes === 0) {
          i = 0
        }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.drawImage(loader.img["defaultStateImg"], imgPro.sleepImgStyle[i].x, imgPro.sleepImgStyle[i].y, imgPro.sleepImgStyle[i].w, imgPro.sleepImgStyle[i].h, imgPro.sleepImgStyle[i].l, imgPro.sleepImgStyle[i].t, imgPro.sleepImgStyle[i].w, imgPro.sleepImgStyle[i].h);
        if(anmTimes < 3) {
          i++
        }else {
          i--
      ...
      if(girlState === 2 && !force) {
        return
      }
      if(goSleep && !hasMsg || goSleep && hasMsg && girlState === 2) {
        girlState = 5;
        removeClass(hideBtn, "show-btn");
        if(canvasSurport) {
          clearTimeout(defaultTime);
          ...
          qgirlContain.className = "qGirl-contain sleep-state"
        }
      }else {
        if(!goSleep && !hasMsg) {
          defaultState()
    ...
    window.advPopMsg = function() {
      if(girlState !== 1 && girlState !== 3 && girlState !== 4) {
        hasMsg = Math.floor(Math.random() * 3) + 1;
        hasReadMsg = 0;
        if(isHidden == 1) {
          showDate(advMsg, 2)
        }else {
    ...
    var adjustPic = function(pic) {
      var picList = [], sizeMap = {1:350, 2:160, 3:110, 4:110, 5:110, 6:110, 7:110, 8:110, 9:110}, wrap = $$("girlMsgPop");
      if(pic.length > 1) {
        for(var i = 0, len = pic.length;i < len;i++) {
          (function(_this, i) {
            picList[i] = new Image;
            picList[i].onload = function() {
              if(picList[i].width <= picList[i].height) {
                wrap.getElementsByTagName("img")[i].style.cssText = "width:" + sizeMap[len] + "px;height:auto;margin-top:" + (sizeMap[len] * picList[i].height / picList[i].width - sizeMap[len]) / -2 + "px"
              }else {
                wrap.getElementsByTagName("img")[i].style.cssText = "height:" + sizeMap[len] + "px;width:auto;margin-left:" + (sizeMap[len] * picList[i].width / picList[i].height - sizeMap[len]) / -2 + "px"
              }
              picList[i].onload = null;
              picList[i] = null
            };
            picList[i].src = pic[i].src
          })(this, i)
        }
      }
    }, clone2 = function(obj) {
      ...
      return o
    }, showDate = function(d, type) {
      var data = d, msgPopHtml = "", msgPop = $$("msgPop"), type = type || 1;
      if(data.id && !data.link) {
        data.link = "http://ac.qq.com/ComicView/index/id/" + data.id + "/seqno/1"
      }
      if(data.pic) {
        if(typeof data.pic == "string") {
          data.pic = [{"src":data.pic}]
        }else {
          for(var i = 0, len = data.pic.length;i < len;i++) {
            data.pic[i] = {"src":data.pic[i]}
          }
        }
      }
      if(data.pic && !data.title && !data.content) {
        data.styleType = 1
      }else {
        if(data.pic && data.title && !data.content) {
          data.styleType = 2
      ...
      if(data.pic && !data.id) {
        addClass(msgPop, "msg-pop-big")
      }else {
        removeClass(msgPop, "msg-pop-big")
      }
      msgPopHtml = template("msgPopMod", data);
      msgPop.innerHTML = msgPopHtml;
      if(data.pic && !(typeof data.pic == "string") && data.pic.length > 1) {
        adjustPic(data.pic)
      }
      hasMsg = Math.floor(Math.random() * 3) + 1;
      if(type === 2) {
    ...
    }, msgLocalRecord = function(message) {
      var newRecord = [message, ID], recordList = JSON.parse(cookie("girlMsgRecord")) || [], total = recordList.length, newRecordList;
      recordList.unshift(newRecord);
      recordList.splice(20);
      newRecordList = JSON.stringify(recordList);
      cookie("girlMsgRecord", newRecordList, {path:"/ComicView/", expires:30})
    }, msgFilter = function(msgList) {
      var recordList = JSON.parse(cookie("girlMsgRecord")) || [], mTotal = msgList.length, rTotal = recordList.length, newRecordList = msgList, advList = newRecordList, comicList = [], seqList = [], seqCount = 0, hasRead = 0;
      for(var i = 0;i < mTotal;i++) {
        if(msgList[i].id || msgList[i].isPop) {
          seqList.push(i);
          comicList.push(msgList[i])
        }
        if(rTotal) {
          for(var j = 0;j < rTotal;j++) {
            if(recordList[j][1] === ID) {
              hasRead = 1
            }
            if(msgList[i].content === recordList[j][0]) {
              seqList.push(i)
            }
          }
        }
      ...
      if(hasRead === 1) {
        ...
        if(seqList.length) {
          for(var k = 0, len = seqList.length;k < len;k++) {
            advList.splice(seqList[k] - seqCount, 1);
            seqCount++
          }
        }
      }
      return[advList, comicList]
    }, handleRecommend = function(data) {
      if(data && +data.ret === 2) {
        var msgList = data.data || [];
        if(data.data.length > 0) {
          if(isCookieSupported && !cookie("topicPop") && msgList[0].isPop === 1 && osHeight > 630) {
            var message = msgList[0];
            showDate(message, 3);
            msgPopState(1);
            sendPgv("NEW.AC_VIEW.GIRL.TOPIC")
          }else {
            var filterData = msgFilter(data.data);
            filterMsgList = filterData[0];
            if(filterMsgList.length > 0) {
              var message = msgList[Math.floor(Math.random() * filterMsgList.length)];
              if(isHidden == 1) {
                sendPgv("NEW.AC_VIEW.girl.shouqi");
                showDate(message, 2)
              }else {
                sendPgv("NEW.AC_VIEW.girl.zhankai");
                showDate(message, 1)
              }
            ...
            advMsgList = filterData[1];
            if(filterMsgList.length === 0 && advMsgList.length > 0 && !NEXT_CHAPTER) {
              window.girlSeq = Math.floor(Math.random() * (maxPage - 1)) + 1;
              var message = advMsgList[Math.floor(Math.random() * advMsgList.length)];
              advMsg = message;
              advPopMsg()
            }
          }
      ...
      var dataSuccess = -1, localData = null;
      if(isLocalStorageNameSupported) {
        localData = window.localStorage.getItem("qGirlRecommend");
        try {
          localData = JSON.parse(localData)
        }catch(e) {
        }
        if(!localData || +localData.expired <= +new Date) {
          localData = "reload"
        }
      }else {
        localData = Math.random() < 0.1 ? "reload" : []
      }
      if(localData === "reload") {
        ajax.get({url:"/ComicView/getQGrilRecommend/", callback:function(data) {
          dataSuccess = 1;
          data = JSON.parse(data);
          if(data && data.ret === 2) {
            data.expired = +new Date + 72E5;
            if(isLocalStorageNameSupported) {
              window.localStorage.setItem("qGirlRecommend", JSON.stringify(data))
            }
            handleRecommend(data)
          }
        }})
      }else {
        handleRecommend(localData)
      }
      setTimeout(function() {
        if(!dataSuccess) {
          hasMsg = 0;
    ...
    if(canvasSurport) {
      var loader = function() {
        this.img = {"defaultStateImg":"http://ac.gtimg.com/media/images/qgirl-default-state-girl_v1.2.1.png"};
        var hasLoaded = 0, imgNum = Object.keys(this.img).length;
        this.total = imgNum;
        function loaded(type, name) {
          if(this[type][name].status !== "loading") {
            return
          }
          this[type][name].status = "loaded";
          hasLoaded++;
          if(typeof this.progress === "function") {
            this.progress(hasLoaded, this.total)
          }
          if(hasLoaded === this.total && typeof this.finished === "function") {
            this.finished()
          }
        }
        this.downloadAll = function() {
          var _this = this;
          var src;
          for(var imgName in this.img) {
            if(this.img.hasOwnProperty(imgName)) {
              src = this.img[imgName];
              (function(_this, imgName) {
                _this.img[imgName] = new Image;
                _this.img[imgName].status = "loading";
                _this.img[imgName].name = img;
                _this.img[imgName].onload = function() {
                  loaded.call(_this, "img", imgName)
                };
                _this.img[imgName].src = src
              })(_this, imgName)
            }
          }
        };
        return{img:this.img, total:this.total, downloadAll:this.downloadAll}
      }();
    ...
    var msgTip = $$("msgTip"), msgPopClose = $$("msgPopClose"), msgPop = $$("msgPop"), msgList = [{"content":"\u4e0d\u8981\u6233\u5566\uff0c\u94f6\u5bb6\u53ea\u60f3\u505a\u4e2a\u5b89\u9759\u6ef4\u7f8e\u5973\u5b50"}, {"content":"\u6233\u5427\u6233\u5427\uff0c\u6233\u6211\u4f60\u4e5f\u4e0d\u4f1a\u6389\u4e8c\u4e24\u8089"}, {"content":"\u53ef\u7231\u7684\u6211\u65e9\u65e9\u8d77\u5e8a\u770b\u6f2b\u753b\uff0c\u4e11\u7684\u4eba\u534a\u591c\u4e0d\u7761\u64b8\u554a\u64b8"}, {"content":"\u5f85\u6211\u5356\u840c\u6210\u5996\uff0c\u5b98\u4eba\u6bcf\u5929\u770b\u6211\u53ef\u597d"}, 
    {"content":"\u5446\u5230\u6df1\u5904\u81ea\u7136\u840c...\u5c31\u8bf4\u4f60\u5462~"}, {"content":"\u6233\u6211\uff01\u6233\u6211\uff01\u5728\u6233\u628a\u4f60\u7c97\u6389\uff01\u256d(\u256f^\u2570)\u256e"}, {"content":"\u6211\u5927\u540d\u5927\u7f8e\u5973\uff0c\u5c0f\u540d\u5c0f\u516c\u4e3e\uff0c\u4e73\u540d\u5b9d\u5b9d\uff0c\u4eba\u79f0\u56fd\u6c11\u751c\u5fc3\u5c0fQ\u5a18"}, {"content":"\u56fd\u6c11\u751c\u5fc3\u5c31\u662f\u6211\uff0c\u6211\u5c31\u662f\u52a8\u6f2b\u5c0fQ\u5a18"}, {"content":"\u8fd8\u6233\uff0c\u5212\u8239\u4e0d\u7528\u6868\u4e48..."}, 
    {"content":"\u4e5f\u4e0d\u77e5\u9053\u5357\u534a\u7403\u7684\u55b5\u51e0\u6708\u4efd\u6389\u6bdb\uff0c\u64cd\u788e\u4e86\u5fc3..."}, {"content":"\u5fc3\u60c5\u4e0d\u597d\uff0c\u4e0d\u77e5\u9053\u662f\u6ca1\u7761\u597d\u8fd8\u662f\u6ca1\u5403\u9971\u54ce\u256e(\u256f\u25bd\u2570)\u256d"}, {"content":"\u6ca1\u4ec0\u4e48\u4e8b\u4e0d\u8981\u968f\u4fbf\u6233\u6211"}, {"content":"Q\u5a18\u5982\u6b64\u591a\u5a07\uff0c\u5f15\u65e0\u6570\u6f2b\u8ff7\u6298\u5728\u6211\u7684\u5c0f\u86ee\u8170"}, {"content":"\u54ce~ \u4eba\u751f\u82e6\u77ed\uff0c\u5fc5\u987b\u6027\u611f"}, 
    {"content":"\u591a\u53d1\u5f39\u5e55\u4f1a\u53d8\u6f02"}, {"content":"\u8981\u4e0d\u8981\u6765\u4e00\u53d1\u989c\u503c\u7206\u8868\u5f39"}, {"content":"\u5b9d\u5b9d\u5fc3\u91cc\u82e6\uff0c\u5b9d\u5b9d\u6bcf\u5929\u53d1\u5f39\u5e55"}, {"content":"\u5f39\u662f\u5f39\u5e55\u7684\u5e55\uff0c\u5e55\u662f\u5f39\u5e55\u7684\u5f39"}, {"content":"\u4f60\u4e0d\u6765\uff0c\u6f2b\u753b\u5c31\u5728\u8fd9\u91cc\uff1b\u4f60\u6765\uff0c\u987a\u4fbf\u53d1\u4e2a\u5f39\u5e55\u6492..."}, {"content":"\u4e00\u6761\u5f39\u5e55\u98d8\u7740\u98d8\u7740\u5c31\u997f\u4e86\uff0c\u6233\u6233\u5c31\u6ee1\u8840\u590d\u6d3b\u4e86"}, 
    {"content":"\u7528\u7231\u5c41\u5c41\u770b\u52a8\u6f2b\uff0c\u5728\u5e8a\u4e0a\u5c31\u80fd\u770b\uff0cTA\u597d\u4f60\u4e5f\u597d"}, {"content":"\u542c\u8bf4\uff0c\u52a8\u6f2b\u7231\u5c41\u5c41\u91cc\u9762\u597d\u591a\u4e09\u56f4\u7206\u8868\u6ef4\u840c\u59b9\u7eb8\u54e6~"}, {"content":"\u4e0b\u8f7d\u52a8\u6f2b\u7231\u5c41\u5c41\u6da8\u59ff\u52bf\u54e6~"}, {"content":"\u8bbe\u8ba1\u6211\u7684\u4ea7\u54c1\u6c6a\u53eb\u8428\u6469\uff0c\u6ca1\u9519\uff0c\u5c31\u662f\u96ea\u767d\u96ea\u767d\u7684\u8fa3\u4e2a"}, 
    {"content":"\u5f00\u53d1\u6211\u7684\u7a0b\u5e8f\u733f\u53eb\u8001\u738b\uff0c\u5c31\u662f\u9694\u58c1\u7684\u8fa3\u4e2a"}, {"content":"\u6211\u624d\u4e0d\u662f\u5145\u8bdd\u8d39\u9001\u7684\uff0c\u6211\u53ef\u662f\u5c04\u51fb\u6e7fMM\u4eb2\u751f\u7684"}, {"content":"\u4e3a\u6211\u5199\u7a0b\u5e8f\u7684\u6bd4\u6211\u8fd8\u840c\u54d2\u5c0f\u5e08\u59b9\u771f\u771f\u662f\u4e3a\u6211\u64cd\u788e\u4e86\u5fc3\u5450"}, {"content":"\u4f60\u8981\u662f\u4e0d\u6765\u770b\u6211\u4e86\uff0c\u53cb\u8c0a\u7684\u5c0f\u8239\u4f1a\u4e0d\u4f1a\u7ffb"}, 
    {"content":"Q\u5a18\u4e00\u76f4\u5728\u627f\u53d7\u6211\u8fd9\u4e2a\u5e74\u7eaa\u4e0d\u8be5\u6709\u7684\u53ef\u7231\u548c\u53ef\u7231\uff0c\u6211\uff0c\u597d\uff0c\u7d2f"}, {"content":"\u4ece\u524d\u8ffd\u6f2b\u50cf\u6761\u72d7\uff0c\u5982\u4eca\u770b\u5b8c\u5acc\u6211\u4e11"}, {"content":"\u5927\u5bb6\u90fd\u662f\u6210\u5e74\u4eba\u4e86\uff0c\u4ec0\u4e48\u4e8b\u60c5\u4e0d\u80fd\u770b\u6f2b\u753b\u89e3\u51b3\uff1f\u4e00\u5b9a\u8981\u4ee5\u5435\u67b6\u5206\u624b\uff01"}, {"content":"\u65b0\u8ffd\u7684\u6f2b\u753b\u7537\u670b\u53cb\u4e0d\u6ee1\u610f\uff0cQ\u5a18\u60f3\u4e86\u4e0b\uff0c\u51b3\u5b9a\u6362\u4e2a\u7537\u670b\u53cb"}, 
    {"content":"\u522b\u518d\u6233\u5566\uff0cQ\u5a18\u4e0d\u80fd\u5bf9\u4e0d\u8d77\u4ef2\u57fa\u6b27\u5df4..."}, {"content":"Q\u5a18\u7684BOSS\u662f\u4e00\u4e2a\u53eb\u98da\u54e5\u7684\u6c49\u7eb8\uff0c\u4ed6\u603b\u662f\u529d\u8bf4\u5927\u5bb6\u8868\u7070\u5fc3\uff0c\u5fd9\u5b8c\u8fd9\u4e00\u9635\u5b50\uff0c\u5c31\u53ef\u4ee5\u5fd9\u4e0b\u4e00\u9635\u5b50\u4e86\uff0c\u5929\u5566\u64b8..."}], girlShow = function() {
      if(hasMsg && !hasReadMsg) {
        msgState()
      }else {
        if(fullMode) {
      ...
      sendPgv("NEW.AC_VIEW.girl.click");
      if(girlState === 3 || girlState === 2 && hasMsg && !hasReadMsg) {
        hasMsg = Math.floor(Math.random() * 3) + 1;
        msgPopState(hasMsg);
        msgLocalRecord(systemMsg)
      }else {
        if(girlState === 0 && !fullMode) {
          var message = msgList[Math.floor(Math.random() * msgList.length)];
          showDate(message);
          hasMsg = Math.floor(Math.random() * 3) + 1;
          msgPopState(hasMsg)
        }else {
          if(girlState === 2) {
            cookie("girlHideState", 0, {path:"/", expires:30});
            sendPgv("NEW.AC_VIEW.girl.zhankai");
            sendPgv("NEW.AC_VIEW.girl.zhankai-button");
            girlShow()
          }
        }
      }
    };
    on(hideBtn, mouse.click, function() {
      if(girlState === 0 || girlState === 1 || girlState === 3 || girlState === 5) {
        sendPgv("NEW.AC_VIEW.girl.shouqi");
        sendPgv("NEW.AC_VIEW.girl.shouqi-button");
        cookie("girlHideState", 1, {path:"/", expires:30});
        if(hasMsg) {
          msgTip.style.display = "none"
        }
        hideState(1)
    ...
    if(msgTip) {
      on(msgTip, mouse.click, girlClick)
    }
    if(canvasSurport) {
      on(canvas, mouse.click, girlClick)
    }else {
      on(qgirlContain, mouse.click, girlClick)
    }
    on(msgPop, mouse.click, function(e) {
      e = e || window.event;
      e = e.target || e.srcElement;
      if(e.id === "msgPopClose") {
        hasMsg = 0;
        ...
        hideState(1);
        msgPop.style.display = "none";
        if(e.getAttribute("data-pop")) {
          cookie("topicPop", "1", {path:"/", expires:1})
        }
      }else {
        if($$("msgPopClose").getAttribute("data-pop") == 1) {
          sendPgv("NEW.AC_VIEW.girl.topic.click")
        }else {
          if(systemMsg) {
            sendPgv("NEW.AC_VIEW.girl.ads.click")
          }else {
            if(advMsg) {
              sendPgv("NEW.AC_VIEW.girl.book.click")
            }
          }
        }
      }
    ...
    $$("qgirlCanvas").parentNode.style.display = "none"
  }
}();
